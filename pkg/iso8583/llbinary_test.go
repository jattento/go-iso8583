...
func TestLLBINARY_MarshalISO8583(t *testing.T) {
	testList := []struct {
		Name        string
		V           iso8583.LLBINARY
		Encoding    string
	...
	for _, testCase := range testList {
		t.Run(fmt.Sprintf("llbinary_to_bytes_%s", testCase.Name), func(t *testing.T) {
...
func TestLLBINARY_UnmarshalISO8583(t *testing.T) {
	testList := []struct {
		Name          string
		InputBytes    []byte
